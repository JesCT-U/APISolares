<div>
    ...
    <div class="py-8 px-16">
        <div class="px-8 pt-8 pb-4 bg-white rounded-lg">
            ...
            @if($open)
                @include('livewire.reabastecimientos.reabastecimiento-crear')
            @endif
            ...
            <table class="w-full text-center">
                <thead class="text-white">
                    <tr>
                        ...
                        <th class="py-6 bg-gray-700">Producto</th>
                        <th class="py-6 bg-gray-700">Unidades adquiridas</th>
                        <th class="py-6 bg-gray-700">Total</th>
                        <th class="py-6 bg-gray-700">fecha compra</th>
                        <th class="py-6 bg-gray-700">proveedor</th>
                        <th class="py-6 bg-gray-700 rounded-tr-lg">Acciones</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($datos as $reabastecimiento)
                        <tr>
                            <td class="py-6 px-2 bg-gray-700 text-white">{{$reabastecimiento->reabastecimiento_id}}</td>
                            <td class="py-6 px-2 bg-gray-300">{{$reabastecimiento->producto}}</td>
                            <td class="py-6 px-2 bg-gray-300">{{$reabastecimiento->unidades}}</td>
                            <td class="py-6 px-2 bg-gray-300">{{$reabastecimiento->Total}}</td>
                            <td class="py-6 px-2 bg-gray-300">{{$reabastecimiento->fecha}}</td>
                            <td class="py-6 px-2 bg-gray-300">{{$reabastecimiento->proveedor}}</td>
                            <td class="py-6 px-2 bg-gray-300">
                                <button wire:click="eliminar({{$reabastecimiento->reabastecimiento_id}})" type="button" class="text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">Eliminar</button>
                                <button wire:click="editar({{$reabastecimiento->reabastecimiento_id}})" type="button" class="text-white bg-gradient-to-r from-pink-400 via-pink-500 to-pink-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-pink-300 dark:focus:ring-pink-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">Editar</button>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
